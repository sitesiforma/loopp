"use client";

import { useParams } from "next/navigation";
import Link from "next/link";
import { motion } from "framer-motion";
import { ArrowLeft, Recycle, Wind, TreePine, Package } from "lucide-react";
import { Button } from "@/components/ui/button";
import Logo from "@/components/Logo";
import { CASES_MOCK } from "@/lib/mock-data";
import { formatNum } from "@/lib/impact-calculator";

const IMPACTO_ITEMS = [
  {
    key: "kgReaproveitado" as const,
    label: "Kg de material reaproveitado",
    icon: Recycle,
    cor: "#2D6A4F",
    sufixo: "kg",
  },
  {
    key: "co2Evitado" as const,
    label: "Kg de CO₂ evitado",
    icon: Wind,
    cor: "#4A90D9",
    sufixo: "kg CO₂",
  },
  {
    key: "arvores" as const,
    label: "Árvores equivalentes",
    icon: TreePine,
    cor: "#2D6A4F",
    sufixo: "árvores",
  },
  {
    key: "itensResgatados" as const,
    label: "Itens resgatados do descarte",
    icon: Package,
    cor: "#4A90D9",
    sufixo: "itens",
  },
];

export default function CaseDetailPage() {
  const { slug } = useParams<{ slug: string }>();
  const caso = CASES_MOCK.find((c) => c.slug === slug);

  if (!caso) {
    return (
      <div className="min-h-screen bg-[#F5EDD8] flex items-center justify-center">
        <div className="text-center">
          <p className="text-[#6B7280] mb-4">Case não encontrado.</p>
          <Button render={<Link href="/cases" />} className="bg-[#2D6A4F] text-white rounded-xl">
            Ver todos os cases
          </Button>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#F5EDD8] flex flex-col">
      {/* Nav */}
      <header className="sticky top-0 z-50 border-b border-[#E5D9BF] bg-[#F5EDD8]/95 backdrop-blur-sm">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <Logo size="md" href="/" />
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            <Link href="/vitrine" className="text-[#6B7280] hover:text-[#2D6A4F] transition-colors">
              Vitrine
            </Link>
            <Link href="/cases" className="text-[#2D6A4F] font-semibold">
              Cases
            </Link>
            <Link href="/calculadora" className="text-[#6B7280] hover:text-[#2D6A4F] transition-colors">
              Calculadora
            </Link>
          </nav>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="sm"
              render={<Link href="/login" />}
              className="text-[#6B7280] hover:text-[#1A1A1A]"
            >
              Entrar
            </Button>
            <Button
              size="sm"
              render={<Link href="/cadastro" />}
              className="bg-[#2D6A4F] text-white hover:bg-[#235540] rounded-lg"
            >
              Sou cliente
            </Button>
          </div>
        </div>
      </header>

      <main className="flex-1">
        {/* Hero */}
        <section
          className="relative py-20 flex items-end overflow-hidden min-h-[320px]"
          style={{ backgroundColor: caso.corCard + "18" }}
        >
          <div
            className="absolute inset-0"
            style={{
              background: `radial-gradient(ellipse at 15% 60%, ${caso.corCard}44, transparent 65%)`,
            }}
          />
          <div
            className="absolute top-8 right-16 w-48 h-48 rounded-full border-2 opacity-10"
            style={{ borderColor: caso.corCard }}
          />
          <div
            className="absolute -bottom-10 right-40 w-72 h-72 rounded-full border opacity-5"
            style={{ borderColor: caso.corCard }}
          />

          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 w-full relative z-10">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
            >
              <Link
                href="/cases"
                className="inline-flex items-center gap-1.5 text-sm text-[#6B7280] hover:text-[#2D6A4F] mb-6 transition-colors"
              >
                <ArrowLeft className="h-3.5 w-3.5" /> Todos os cases
              </Link>
              <div className="flex items-center gap-2 mb-4">
                <span
                  className="text-xs font-bold px-2.5 py-1 rounded-full text-white"
                  style={{ backgroundColor: caso.corCard }}
                >
                  {caso.tipo}
                </span>
                <span className="text-sm text-[#6B7280]">{caso.cliente} · {caso.ano}</span>
              </div>
              <h1
                className="text-4xl sm:text-5xl font-bold text-[#1A1A1A] leading-tight"
                style={{ fontFamily: "var(--font-fraunces)" }}
              >
                {caso.evento}
              </h1>
            </motion.div>
          </div>
        </section>

        {/* Conteúdo editorial */}
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-14 space-y-12">
          {/* Desafio */}
          <motion.section
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
          >
            <h2
              className="text-xs font-bold uppercase tracking-widest text-[#6B7280] mb-3"
            >
              O desafio
            </h2>
            <p
              className="text-xl text-[#1A1A1A] leading-relaxed"
              style={{ fontFamily: "var(--font-fraunces)" }}
            >
              {caso.desafio}
            </p>
          </motion.section>

          {/* Solução */}
          <motion.section
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
          >
            <h2 className="text-xs font-bold uppercase tracking-widest text-[#6B7280] mb-3">
              A solução
            </h2>
            <p className="text-lg text-[#1A1A1A] leading-relaxed mb-5">
              {caso.solucao}
            </p>
            <div className="flex flex-wrap gap-2">
              {caso.fornecedores.map((f) => (
                <span
                  key={f}
                  className="text-sm bg-[#2D6A4F]/10 text-[#2D6A4F] border border-[#2D6A4F]/20 px-3 py-1 rounded-full font-medium cursor-default hover:bg-[#2D6A4F]/15 transition-colors"
                >
                  {f}
                </span>
              ))}
            </div>
          </motion.section>

          {/* Impacto */}
          <motion.section
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
          >
            <h2 className="text-xs font-bold uppercase tracking-widest text-[#6B7280] mb-5">
              Impacto
            </h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {IMPACTO_ITEMS.map((item) => {
                const Icon = item.icon;
                const valor = caso.impacto[item.key];
                return (
                  <div
                    key={item.key}
                    className="bg-white rounded-2xl p-5 border border-[#E5D9BF] shadow-[0_2px_16px_rgba(0,0,0,0.05)] text-center"
                  >
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-3"
                      style={{ backgroundColor: item.cor + "18" }}
                    >
                      <Icon className="h-5 w-5" style={{ color: item.cor }} />
                    </div>
                    <p
                      className="text-2xl font-bold text-[#1A1A1A] mb-0.5"
                      style={{ fontFamily: "var(--font-fraunces)" }}
                    >
                      {formatNum(valor)}
                    </p>
                    <p className="text-xs text-[#6B7280] leading-tight">
                      {item.label}
                    </p>
                  </div>
                );
              })}
            </div>
          </motion.section>

          {/* Depoimento */}
          <motion.section
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
            className="bg-white rounded-3xl p-10 border border-[#E5D9BF] shadow-[0_2px_16px_rgba(0,0,0,0.05)]"
          >
            <div
              className="text-6xl leading-none text-[#2D6A4F]/20 mb-2 select-none"
              style={{ fontFamily: "var(--font-fraunces)" }}
            >
              "
            </div>
            <p
              className="text-xl text-[#1A1A1A] leading-relaxed mb-5 italic"
              style={{ fontFamily: "var(--font-fraunces)" }}
            >
              {caso.depoimento}
            </p>
            <p className="text-sm font-semibold text-[#2D6A4F]">
              — {caso.autorDepoimento}
            </p>
          </motion.section>

          {/* CTA */}
          <motion.section
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
            className="text-center py-6"
          >
            <h2
              className="text-3xl font-bold text-[#1A1A1A] mb-3"
              style={{ fontFamily: "var(--font-fraunces)" }}
            >
              Quer um evento assim?
            </h2>
            <p className="text-[#6B7280] mb-7">
              Conte sobre o seu evento e receba um plano sustentável personalizado.
            </p>
            <Button
              render={<Link href="/cadastro" />}
              className="bg-[#2D6A4F] hover:bg-[#235540] text-white rounded-xl px-8 h-11 font-semibold text-base shadow-lg shadow-[#2D6A4F]/20"
            >
              Criar conta grátis
            </Button>
          </motion.section>
        </div>
      </main>

      {/* Footer */}
      <footer className="bg-white border-t border-[#E5D9BF] py-8">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex flex-col items-center sm:items-start gap-1">
            <Logo size="sm" />
            <p className="text-sm text-[#6B7280]">Feche o ciclo do seu evento.</p>
          </div>
          <nav className="flex items-center gap-5 text-sm text-[#6B7280]">
            <Link href="/" className="hover:text-[#2D6A4F] transition-colors">Início</Link>
            <Link href="/vitrine" className="hover:text-[#2D6A4F] transition-colors">Vitrine</Link>
            <Link href="/calculadora" className="hover:text-[#2D6A4F] transition-colors">Calculadora</Link>
          </nav>
          <p className="text-xs text-[#6B7280]/60">© 2026 Loopp</p>
        </div>
      </footer>
    </div>
  );
}
